...
------------------
Everything should work as of rigght now, except the popups. The popups are pulling up all of the data for the crashes at a location. But, it's pulling it up
in a one new popup. The goal is to make it display one crash at a time instead of displaying all of the crashes in the same popup then to use the html buttons
to sort through the popups. Currently there is no code to progress through the features (probably something like +1 to feature.length or something with a listener to
pickup the increased count), but I am not able to display the buttons.   